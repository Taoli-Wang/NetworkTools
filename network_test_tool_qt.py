...
class ConfigManager:
    ...
    DEFAULT = {
        "gateway_targets": [["192.168.1.1", "示例网关"]],
        "server_targets": [["10.0.0.1", "示例服务器"]],
...
class MainWindow(QMainWindow):
    ...
    def _show_about(self):
        ...
        title = QLabel(f"Network Tools {ABOUT_INFO['version']}")
        ...
        desc = QLabel(
            "一款面向 IT 运维的轻量级网络测试工具。\n\n"
            "功能：ICMP Ping / TCP 端口检测 / 路由追踪\n"
            "子网计算 / 局域网扫描 / IP 地址批量管理\n\n"
            "开源协议：MIT License\n"
            "纯 Python + PyQt5 实现"
        )
        desc.setStyleSheet("font-size:11px; color:#334155; padding:10px;")
        desc.setAlignment(Qt.AlignCenter)
        layout.addWidget(desc)
        layout.addStretch()

        footer = QLabel("Copyright (C) 2026  Open Source Project")
